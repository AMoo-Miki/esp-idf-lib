...
esp_err_t ssd1306_init(ssd1306_t *dev)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_display_on(dev, false));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_osc_freq(dev, 0x80));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_charge_pump_enabled(dev, true));
    if (dev->chip == SH1106_CHIP)
        CHECK(sh1106_set_charge_pump_voltage(dev, SH1106_VOLTAGE_74));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_mux_ratio(dev, mux_ratio));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_display_offset(dev, 0x0));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_display_start_line(dev, 0x0));
    if (dev->chip == SSD1306_CHIP)
        ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_mem_addr_mode(dev, SSD1306_ADDR_MODE_HORIZONTAL));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_segment_remapping_enabled(dev, false));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_scan_direction_fwd(dev, true));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_com_pin_hw_config(dev, pin_cfg));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_contrast(dev, 0x9f));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_precharge_period(dev, 0xf1));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_deseltct_lvl(dev, 0x40));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_inversion(dev, false));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_set_display_mode(dev, SSD1306_MODE_NORMAL));
    ESP_ERROR_CHECK_WITHOUT_ABORT(ssd1306_display_on(dev, true));
...
esp_err_t ssd1306_draw_char(ssd1306_t *dev, uint8_t x, uint8_t y, char c, const ssd1306_font_t *font, ssd1306_color_t foreground, ssd1306_color_t background, uint8_t *width)
{
    ...
    for (j = 0; j < font->height; ++j)
    {
        for (i = 0; i < d->width; ++i)
        {
            ...
            if (line & 0x80)
            {
                CHECK(ssd1306_set_pixel(dev, x + i, y + j, foreground));
            }
            else
            {
                switch (background)
                {
                    case OLED_COLOR_TRANSPARENT:
                        ...
                        break;
                    default:
                        CHECK(ssd1306_set_pixel(dev, x + i, y + j, background));
                        break;
                }
            }
...
{
    CHECK_ARG(dev && dev->fb && font && str && x < dev->width && y < dev->height);

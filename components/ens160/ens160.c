...
esp_err_t ens160_init(i2c_dev_t *dev)
{
    CHECK_ARG(dev);
    ...
    if (ens160_set_mode(dev, ENS160_MODE_STD) != ESP_OK) {
        res = ESP_FAIL;
        goto err;
    }
...
esp_err_t ens160_reset_baseline(i2c_dev_t *dev, ens160_bl_err_t* err)
{
    ...
    if (ens160_set_mode(dev, ENS160_MODE_BOOTLOADER) != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t ens160_versions(i2c_dev_t *dev, uint8_t (*fw_version)[3], uint8_t (*hw_version)[2])
{
    ...
    if (ens160_set_mode(dev, ENS160_MODE_IDLE) != ESP_OK) {
        return ESP_FAIL;
    }
...
esp_err_t ens160_reset(i2c_dev_t *dev)
{
    ...
    uint8_t data = ENS160_MODE_RESET;
